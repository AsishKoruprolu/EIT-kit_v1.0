...
EITKitArduino::EITKitArduino(int num_electrodes, int num_bands, int num_terminals, Meas_t drive_type, Meas_t meas_type, bool bluetooth_communication)
{
  ...
  vspi->begin(VSPI_SCK_PIN, MISO, VSPI_MOSI_PIN); //This line has been added (not there in original code)
  IOExpander.begin();
  vspi->setHwCs(false);
  ...
  //Test to check GPIO/ADC read capabilities
  Serial.println("GPIO read test, "+String(gpio_read(),HEX));
  ...
  // calibrateEIT(); commented for our debug case

  uint16_t i;

  /* Read resting impedance state for calibration (Commented for our EIT debug case) */
  for (i = 0; i < 30; i++)
  {
...
void EITKitArduino::mux_write(const int chip_select, uint8_t pin_sel, uint8_t enable){
  ...
  //  Serial.println("MUX Code: "+String(value));
...
void EITKitArduino::mux_write_to_electrode(Mux_t chip_select, uint8_t electrode_sel, uint8_t enable){
  if(electrode_sel<32){
    ...
    Serial.println("CS: "+String(cs_pin));
...
void EITKitArduino::read_frame_for_band(uint8_t band, Meas_t drive_type, Meas_t meas_type, double * rms_array, uint8_t electrodes_per_band){
  ...
  uint8_t src_pin=0, sink_pin=0, vp_pin=0, vn_pin=0;
  uint16_t num_meas = 0;
